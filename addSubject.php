...
<?php
    ...
    if(isset($_GET['id']) && !empty($_GET['id'])){
        $searchNews = $db->query('SELECT * FROM frm_subjects WHERE id = "'.$_GET['id'].'"');
                ...
                if(isset($_POST['formAddNews'])){
                    if(!empty($_POST['titleAdd']) && !empty($_POST['contentAdd'])){
                        if(strlen($_POST['titleAdd']) <= 80) {
                            ...
                            $updateNews = $db->prepare('UPDATE frm_subjects SET title = :title, content = :content WHERE id = "'.$news['id'].'"');
                            $updateNews->execute(array(
                                'title'     => $titleAdd,
                                'content'   => $contentAdd
                            ));
                            header('Location: forum.php?id='.$news['id']);
                        } else {
        ...
        if(isset($_POST['formAddNews'])){
            if(!empty($_POST['titleAdd']) && !empty($_POST['contentAdd'])){
                ...
                if(strlen($titleAdd) <= 80) {
                    ...
                    $insertNew = $db->prepare('INSERT INTO frm_subjects(title, content, date_publication, id_user) VALUES(:title, :content, :date_publication, :id_user)');
                    ...
                    $searchRedirect = $db->query('SELECT * FROM frm_subjects ORDER BY id DESC LIMIT 0, 1');
                    $redirect = $searchRedirect->fetch(PDO::FETCH_ASSOC);
                    $red = $redirect['id'];
                    header('Location: forum.php?id='.$red);
                } else {
                    $error = "Le titre ne doit pas dépasser 80 caractères.";
                }
            } else {
...
?>
<title><?= $infosSite['websiteName']; ?> - Créer/Modifier un topic</title>
...
<div class="container">
    ...
    <div class="firstBlock">
        <form method="POST" action="">
            ...
            <input type="submit" name="formAddNews" value="Publier le topic">
        </form>
        <div class="error">
            <?php if(isset($error)){
                echo $error;
